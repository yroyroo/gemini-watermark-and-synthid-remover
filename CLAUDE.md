...
- **Known limitation** (1.8.0): LaMa reconstructs the most intricate backgrounds far better than FSR/NS (the Phase-C win), but the fill is **soft on cartoon/line-art** — big-lama was trained on Places2 (natural photos), so flat-color cartoon detail (e.g. a character's mouth crossing the mark) is synthesized as a plausible-but-soft reconstruction, not the exact crisp line (which is also partially info-lost under the semi-transparent mark). A post-fill unsharp mask crisps it but adds halo/noise — **evaluated + rejected** (ship as-is). Outside the hole, LaMa preserves the original at ~x264-noise level. Runtime ~2.4 s/frame CPU, bounded by complexity-gating (only the hardest scenes pay it) — infeasible as a default, fine opt-in / for short clips.
